  ...
  return (
    <article className="rounded-[1.75rem] border border-[#24344d] bg-[linear-gradient(180deg,#0f1a2c,#0a1321)] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.16)]">
      <div className="flex items-start gap-4">
        <div className="overflow-hidden rounded-[1.25rem] border border-[#2f425d] bg-[#132339]">
          <Image
            src={comment.photo || "/profile.png"}
            alt={comment.impression || "Comment photo"}
            width={60}
            height={60}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-base font-semibold text-white">{comment.impression || "Anonymous"}</p>
              <p className="mt-1 text-xs uppercase tracking-[0.28em] text-[#7adab5]">Community review</p>
            </div>
...
export default function ServiceCommentsPanel({ serviceSlug, comments = [] }) {
  ...
  useEffect(() => {
    ...
    fetch(buildPublicApiUrl("/api/admin/me"), {
      ...
      .then(async (response) => {
        if (!response.ok) {
          throw new Error("Admin session not active.");
        }
  ...
  useEffect(() => {
    ...
    socket.emit("service:join", serviceSlug);

    socket.on("service:comment_created", (payload) => {
      if (payload?.serviceSlug !== serviceSlug || !payload.comment) {
        return;
      }
    ...
    socket.on("service:reply_created", (payload) => {
      if (payload?.serviceSlug !== serviceSlug || !payload.reply) {
        return;
      }
    ...
    return () => {
      socket.emit("service:leave", serviceSlug);
      socket.disconnect();
    };
  }, [serviceSlug]);
  ...
  async function handleReplySubmit(event, commentId) {
    ...
    if (!replyText) {
      setReplyStatus((current) => ({
        ...current,
        [commentId]: createStatus("error", "Reply is required."),
      }));
      return;
    }
    ...
    if (!admin && !replyName) {
      setReplyStatus((current) => ({
        ...current,
        [commentId]: createStatus("error", "Name is required for replies."),
      }));
      return;
    }
  ...
  return (
    <section className="mt-14 rounded-[2rem] border border-[#203049] bg-[linear-gradient(180deg,rgba(11,18,31,0.92),rgba(8,13,23,0.98))] p-6 shadow-[0_28px_80px_rgba(0,0,0,0.22)] md:p-8">
      <div className="flex flex-col gap-4 border-b border-[#1d2d42] pb-6 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-[#79d4ff]">Client Discussion</p>
          <h2 className="mt-3 text-2xl font-semibold text-white md:text-3xl">Feedback and replies</h2>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          {admin ? (
            <div className="rounded-2xl border border-[#315a4b] bg-[#10241c] px-4 py-3 text-sm text-[#aef2cf]">
              Replying as {admin.name}
            </div>
          ) : null}
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1.2fr)_360px]">
                      ...
                      {comment.replies.map((reply) => (
                        ...
                        >
                          <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                            <p className="text-sm font-semibold text-[#dfeaf7]">{reply.impression || "Reply"}</p>
                    ...
                    {openReplyEditorId === comment.id ? (
                      <form className="mt-4" onSubmit={(event) => handleReplySubmit(event, comment.id)}>
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                          <p className="text-xs uppercase tracking-[0.28em] text-[#79d4ff]">
                            {admin ? "Admin reply" : "Post a reply"}
                          </p>
                          {!admin ? (
                            <input
                              className="w-full rounded-xl border border-[#2c3852] bg-[#101b2d] px-4 py-3 text-sm text-white outline-none transition focus:border-[#49c1ff] md:max-w-[220px]"
                              value={activeReplyForm.name}
                              onChange={(event) => updateReplyForm(comment.id, { name: event.target.value })}
                              placeholder="Your name"
                            />
                          ) : (
                            <p className="text-sm text-[#8fb3cf]">Signed in as {admin.name}</p>
                          )}
                        </div>
        ...
        <div className="h-fit rounded-[1.75rem] border border-[#24344d] bg-[linear-gradient(180deg,#101a2c,#0b1422)] p-5 xl:sticky xl:top-8">
          ...
          <h3 className="mt-3 text-2xl font-semibold text-white">Add your feedback</h3>
          <div className="mt-6">
            <button
              type="button"
