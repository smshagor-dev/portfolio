      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs uppercase tracking-[0.28em] text-[#7adab5]">Research discussion</p>
          </div>
...
export default function ResearchCommentsPanel({ publicationSlug, comments = [] }) {
  ...
  useEffect(() => {
    ...
    socket.emit("research:join", publicationSlug);

    socket.on("research:comment_created", (payload) => {
      if (payload?.publicationSlug !== publicationSlug || !payload.comment) {
        return;
      }
    ...
    socket.on("research:reply_created", (payload) => {
      if (payload?.publicationSlug !== publicationSlug || !payload.reply) {
        return;
      }
    ...
    return () => {
      socket.emit("research:leave", publicationSlug);
      socket.disconnect();
    };
  }, [publicationSlug]);
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const response = await fetch(
        buildPublicApiUrl(`/api/research-publications/${encodeURIComponent(publicationSlug)}/comments`),
        {
          method: "POST",
          headers: {
  ...
  async function handleReplySubmit(event, commentId) {
    ...
    try {
      ...
      const response = await fetch(
        buildPublicApiUrl(`/api/research-publications/${encodeURIComponent(publicationSlug)}/comments/${commentId}/replies`),
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <section className="mt-14 rounded-[2rem] border border-[#203049] bg-[linear-gradient(180deg,rgba(11,18,31,0.92),rgba(8,13,23,0.98))] p-6 shadow-[0_28px_80px_rgba(0,0,0,0.22)] md:p-8">
      <div className="flex flex-col gap-4 border-b border-[#1d2d42] pb-6 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-[#79d4ff]">Research Discussion</p>
      ...
      <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1.2fr)_360px]">
        ...
        <div className="h-fit rounded-[1.75rem] border border-[#24344d] bg-[linear-gradient(180deg,#101a2c,#0b1422)] p-5 xl:sticky xl:top-8">
          ...
          <h3 className="mt-3 text-2xl font-semibold text-white">Join the discussion</h3>
          <div className="mt-6">
            <button
              type="button"
              ...
              <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Write your thoughts about this publication..."
                  />
                </div>
