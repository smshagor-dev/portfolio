...
export default function ResearchCardMetrics({
  slug,
  views = 0,
  impressions = 0,
  discussionCount = 0,
  ...
  useEffect(() => {
    ...
    const sessionKey = `research-impression:${slug}`;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        fetch(buildPublicApiUrl(`/api/research-publications/${encodeURIComponent(slug)}/impression`), {
          method: "POST",
          headers: {
  ...
  return (
    <div ref={cardRef} className={`flex flex-wrap items-center gap-2 ${className}`}>
      ...
      <MetricItem title="Comments" value={discussionCount || 0}>
        <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 fill-none stroke-current" strokeWidth="1.8">
