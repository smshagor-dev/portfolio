...
export default function ResearchHeaderMetrics({
  publicationSlug,
  views = 0,
  impressions = 0,
  discussionCount = 0,
  ...
  useEffect(() => {
    if (!publicationSlug || typeof window === "undefined") {
      return;
    }

    const sessionKey = `research-header-impression:${publicationSlug}`;
    ...
    fetch(buildPublicApiUrl(`/api/research-publications/${encodeURIComponent(publicationSlug)}/impression`), {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    function handleShare(event) {
      if (event.detail?.publicationSlug !== publicationSlug) {
        return;
      }
    ...
    window.addEventListener("research:share-tracked", handleShare);
    return () => window.removeEventListener("research:share-tracked", handleShare);
  }, [publicationSlug]);
  ...
  return (
    <div className={containerClass}>
      ...
      <MetricComponent title="Comments" value={discussionCount || 0}>
        <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 fill-none stroke-current" strokeWidth="1.8">
