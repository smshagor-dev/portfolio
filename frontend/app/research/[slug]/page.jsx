...
function extractFullAbstract(content, fallback) {
  const normalized = String(content || "").replace(/\r\n/g, "\n").trim();
  const fallbackText = String(fallback || "").trim();

  if (!normalized || /<([a-z][\w-]*)(?:\s[^>]*)?>/i.test(normalized)) {
    return fallbackText;
  }

  const lines = normalized.split("\n").map((line) => line.trim());
  const sectionHeadings = new Set(["publication details", "authors", "external identifiers"]);
  const abstractLines = [];
  let collecting = false;

  for (const line of lines) {
    if (!line) {
      if (collecting && abstractLines.length) {
        abstractLines.push("");
      }
      continue;
    }
    ...
    if (!collecting) {
      ...
      if (lower.startsWith("abstract:")) {
        collecting = true;
        const inlineAbstract = line.slice(line.indexOf(":") + 1).trim();
        if (inlineAbstract) {
          abstractLines.push(inlineAbstract);
        }
      }
      continue;
    }
  ...
  const fullAbstract = abstractLines.join("\n").replace(/\n{3,}/g, "\n\n").trim();
  return fullAbstract || fallbackText;
}
...
export default async function ResearchDetailPage({ params }) {
  ...
  const fullAbstract = extractFullAbstract(publication.content, publication.shortSummary);
  const canonicalUrl = settings?.canonicalUrl
  ...
  const schema = {
    ...
    description: fullAbstract || publication.shortSummary,
      ...
      <section className="relative mt-5 overflow-hidden rounded-[2.2rem] border border-[#22344d] bg-[radial-gradient(circle_at_85%_10%,rgba(112,213,255,0.16),transparent_27%),linear-gradient(160deg,#101c31,#091321_58%,#07101c)] shadow-[0_30px_90px_rgba(0,0,0,0.28)]">
        <div className="grid xl:grid-cols-[minmax(0,1fr)_390px]">
          <div className="p-6 sm:p-8 lg:p-10">
            ...
            <h1 className="mt-6 max-w-5xl break-words text-3xl font-semibold leading-[1.12] text-white sm:text-4xl lg:text-5xl">
              {publication.title}
            </h1>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 rounded-[1.4rem] border border-[#29405b] bg-[#091525]/78 p-4">
              ...
              {pdf?.url ? (
                <>
                  <ActionLink href="#research-pdf">Read PDF</ActionLink>
            ...
            <div className="mt-6 rounded-[1.5rem] border border-[#29405b] bg-[#0a1626]/88 p-5 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-full border border-[#28415c] px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-[#7f9bb4]">Full abstract</span>
              </div>
              <div className="mt-4 whitespace-pre-line break-words text-sm leading-8 text-[#c9d5e2] sm:text-base">
                {fullAbstract || "An abstract has not been indexed for this record yet."}
              </div>
            </div>
          </div>
          ...
          <aside className="border-t border-[#1d2d42] bg-[#08111f]/72 p-6 sm:p-8 xl:border-l xl:border-t-0">
            ...
            <div className={`${publication.thumbnailImage ? "mt-6" : ""} rounded-[1.5rem] border border-[#243850] bg-[#0b1626] p-5`}>
